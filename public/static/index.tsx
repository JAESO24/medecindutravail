...
import { sendEmail, welcomeEmailHtml } from '../../src/email'
...
app.get('/api/consultations/:id', requireAuth, async (c) => {
  const tenantId = c.get('tenantId')
  const id = c.req.param('id')
  const consultation = await c.env.DB.prepare(`
    SELECT c.*, t.nom, t.prenom, t.poste, e.nom as entreprise,
  ...
  if (!consultation) return c.json({ error: 'Consultation non trouvée' }, 404)

  const constantes = await c.env.DB.prepare(
    'SELECT * FROM constantes WHERE consultation_id = ? ORDER BY date_mesure DESC'
  ).bind(id).first()

  return c.json({ ...(consultation as any), constantes: constantes || null })
})
...
app.delete('/api/users/:id', requireAuth, async (c) => {
  ...
  const id = c.req.param('id')
  if (Number(id) === c.get('userId')) return c.json({ error: 'Vous ne pouvez pas supprimer votre propre compte.' }, 400)

  await c.env.DB.prepare("UPDATE users SET actif=0 WHERE id=? AND tenant_id=?").bind(id, tenantId).run()
  return c.json({ success: true })
})
